---
title: "Civil Society's Effects on Electoral Democracy"
format: 
  html:
    self-contained: true
echo: false
...
ggplot(data = average_indices) + # Invoke ggplot to use geom_point for visualization
  ...
  labs(
    ...
    title = "Figure 1: Mean Civil Society and Electoral Democracy"
  ) +
...
ggplot(data = average_indices_strength_elec, 
  ...
  labs(
    ...
    title = "Figure 2: Mean Civil Society Strength and Electoral Democracy",
...
modelsummary(
  ...
  title = "Table 1: Regression of Electoral Democracy on Civil Society Strength, Repression, and Their Interaction (Main Model)",
...
modelsummary(
  ...
  fmt = 3,
  statistic = "({std.error})",
  title = "Table 5: Regression on Electoral Democracy (Restricted to High GDP Per Capita Countries), including covariates, country-level fixed effects, and lagged repression",
  notes = "Standard errors in parentheses. * p < 0.05, ** p < 0.01, *** p < 0.001. Country fixed effects included but not displayed.",
  coef_map = c(
    "(Intercept)" = "Intercept",
    "civil_society_strength" = "Civil Society Strength",
...
modelsummary(
  ...
  fmt = 3,
  statistic = "({std.error})",
  title = "Table 6: Regression on Electoral Democracy (Restricted to High GDP Per Capita Countries), including a linear time trend, covariates, lagged repression, and country-level fixed effects",
  notes = "Standard errors in parentheses. * p < 0.05, ** p < 0.01, *** p < 0.001. Country fixed effects included but not displayed.",
  coef_map = c(
    "(Intercept)" = "Intercept",
    "civil_society_strength" = "Civil Society Strength",
...
Holding repression, time, and covariates constant, within the same above gdp per-capita median country over time, a one-unit increase in civil society strength is associated with a .151 increase in electoral democracy. It is important to highlight that the interaction term between CSO remains negative and significant. This indicates that the positive relationship between civil society strength and electoral democracy is weakened when accounting for repression that occurred last year. For each 1 unit increase in lagged repression, the marginal effect of civil society strength on electoral democracy decreases by .042. This model accounts for all time invariant differences across countries meaning the interaction term accounts for effects within countries over time. The inclusion of the linear time trend in this model now accounts for secular changes over time like global shifts in civic norms to ensure that the interaction term is not capturing long-term movement in repression and civil society. 

```{r}
pred_data <- expand.grid(
  civil_society_strength = seq(0, 1, by = 0.01),  # Vary CSO strength from 0 to 1
  lag_repression = seq(min(vdem_high_gdp$lag_repression, na.rm = TRUE),
                       max(vdem_high_gdp$lag_repression, na.rm = TRUE),
                       length.out = 100),
  time_trend = median(vdem_high_gdp$time_trend, na.rm = TRUE),
  e_peaveduc = median(vdem_high_gdp$e_peaveduc, na.rm = TRUE),
  v2smgovsmcenprc = median(vdem_high_gdp$v2smgovsmcenprc, na.rm = TRUE),
  country_name = "Germany"  # fixed country to hold FE constant (must be in dataset)
)
```

```{r}
pred_data$predicted_polyarchy <- predict(main_regress_highgdp_time_trend, newdata = pred_data)
```

```{r}
ggplot(pred_data, aes(x = civil_society_strength, y = predicted_polyarchy,
                      color = lag_repression, group = lag_repression)) +
  geom_line(alpha = 0.8) +
  scale_color_viridis_c(option = "magma") +
  labs(
    title = "Figure 5: Predicted Electoral Democracy by CSO Strength and Lagged Repression",
    x = "Civil Society Strength",
    y = "Predicted Electoral Democracy",
    color = "Lagged Repression (t+1)",
    caption = "Note: Shows how the effect of civil scoiety strength weakens when past repression increases"
  ) +
  theme_classic() +
  theme(
